...
export async function scanClient({ baseUrl, model, apiKey, mission, action, image, threshold = 60, webhookAction, webhookSchema, examples, optimizedInstruction, requestTimeout, signal }) {
  ...
  const cfg = { baseUrl: baseUrl.replace(/\/+$/, ''), model, apiKey, timeoutMs: timeoutSec * 1000 };
...
async function callProvider(cfg, system, userText, image, temperature) {
  ...
  try {
    ...
    // Translate our own abort into an actionable message. Without this the
    // browser surfaces the opaque "signal is aborted without reason".
    if (timedOut.hit || err?.name === 'AbortError' || err?.name === 'TimeoutError') {
