...
export async function scanClient({ baseUrl, model, apiKey, mission, action, image, threshold = 60, webhookAction, webhookSchema, examples, optimizedInstruction, requestTimeout, signal }) {
  ...
  const timeoutSec = noTimeout ? null : (Number.isFinite(requestTimeout) && requestTimeout > 0 ? requestTimeout : 30);
  const cfg = { baseUrl: baseUrl.replace(/\/+$/, ''), model, apiKey, timeoutMs: noTimeout ? null : timeoutSec * 1000, signal };
...
async function callProvider(cfg, system, userText, image, temperature) {
  ...
  const timeout = cfg.timeoutMs == null ? null : setTimeout(() => {
    timedOut.hit = true;
  ...
  try {
    ...
    if (timedOut.hit) {
      throw new Error(`Provider request timed out after ${cfg.timeoutMs / 1000}s (auto-tuned from recent response times). The model may be slow or unreachable — try MAX mode for no forced timeout, or switch to a faster provider/model.`);
    }
    throw err;
  } finally {
