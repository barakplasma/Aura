...
import { isOpenRouter, visionModelIds } from "./providers.js";
...
export async function fetchModels(baseUrl, apiKey, { visionOnly = false } = {}) {
  const base = (baseUrl || "").replace(/\/+$/, "");
  if (!base) throw new Error("Base URL is required.");
  let resp;
  try {
    // OpenRouter can filter its live catalogue server-side. It is precise: a
    // selected model is guaranteed to accept the image Aura sends on each scan.
    const suffix = visionOnly && isOpenRouter(base) ? '?input_modalities=image' : '';
    resp = await fetch(`${base}/models${suffix}`, {
  ...
  return visionOnly ? visionModelIds(rows, { assumeVisionWhenUnknown: !isOpenRouter(base) }) : rows.map((m) => m.id).filter(Boolean).sort();
}
  ...
  const { fired, message, webhookMessage, usage } = await runAlertLegs({
    ...
    runLeg: async (leg) => {
      if (leg === "action") {
        ...
        return { message: parseAction(act.content).message, usage: act.usage };
      }
      ...
      return { message: parseWebhookAction(wh.content).message, usage: wh.usage };
    },
  });
...
async function callProvider(cfg, system, userText, image) {
  const body = {
    model: cfg.model,
    messages: [
