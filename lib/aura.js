...
async function callProvider(cfg, system, userText, image, temperature) {
  ...
  if (!jsonModeUnsupported.has(cfg.baseUrl)) {
  ...
  if (!resp.ok) {
    ...
    if (body.response_format && rejectsJsonMode(resp.status, detail)) {
      jsonModeUnsupported.add(cfg.baseUrl);
...
// Base URLs whose server rejected `response_format: json_object`. Session-only
// (module state), so a server that gains support just needs a page reload.
