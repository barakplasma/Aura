...
export async function loadDetector(
  ...
  // A row that prefers WebGPU still loads without it — ORT falls back to
  // WASM, which for a 2.4M-parameter detector is slower but not absurd. The
  // row's `requiresWebGpu` decides which row gets *picked*, not whether it
  // can run at all.
  const device =
