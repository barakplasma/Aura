...
export async function scanBrowser({
  ...
  const infer = (prompt, { schema, maxNewTokens, wantLogits, purpose }) =>
    useChrome
      ? chromeAICall({ prompt, imageDataUrls: frameUrls, schema, signal })
      : send("scan", { prompt, imageDataUrls: frameUrls, maxNewTokens, wantLogits, purpose }, { signal });
  ...
  // A number in the answer is the model's own claim and wins. With no number
  // the parser defaults to 100 when it found a YES, which on the reference
  // phone meant every alert stored conf 100 — including for an object that was
  // not in frame — and the sensitivity slider could not move a verdict. The
  // worker's logits are evidence: the probability this model put on its own
  // first token is P(verdict | frame) for the terse answers it emits.
  if (detMsg.logits?.firstTokenProb != null && !/\d/.test(detMsg.text || "")) {
    detection.confidence = Math.round(100 * detMsg.logits.firstTokenProb);
  }
  ...
  if (fired) {
    if ((action || "").trim()) {
      ...
      const actMsg = await infer(
        ...
        { schema: MESSAGE_SCHEMA, maxNewTokens: budget.action, purpose: "announce" },
      );
      message = json
    ...
    if ((webhookAction || "").trim()) {
      ...
      const whMsg = await infer(
        ...
        { schema: MESSAGE_SCHEMA, maxNewTokens: budget.action, purpose: "webhook" },
      );
