...
function workerCrashError(event) {
  ...
  return browserEngineError(`Browser engine worker crashed: ${detail}`, {
    phase: workerInitialized ? pendingPhase() : "worker-initialization",
...
export async function scanBrowser({
  ...
  const infer = (prompt, { schema, maxNewTokens, wantLogits }) =>
    useChrome
      ? chromeAICall({ prompt, imageDataUrls: frameUrls, schema, signal })
      : send("scan", { prompt, imageDataUrls: frameUrls, maxNewTokens, wantLogits }, { signal });
  ...
  const detMsg = await infer(
    ...
    { schema: DETECTION_SCHEMA, maxNewTokens: budget.detect, wantLogits: true },
  );
