...
// The detector is a second, independent slot in the same worker — see the
// 'detect' task in src/workers/ml.worker.js.
...
export async function loadDetector(modelKey = DEFAULT_DETECTOR_MODEL, { onProgress, signal } = {}) {
  ...
  // A row that wants WebGPU still loads without it — ORT falls back to WASM,
  // which for a 2.4M-parameter detector is slow but not absurd. The row's
  // `requiresWebGpu` drives which row gets *picked*, not whether it can run.
  const device =
    typeof navigator !== "undefined" && navigator.gpu ? "webgpu" : "wasm";
  const promise = send(
    "load",
 ...
 * arithmetic, unit-testable, and out of the worker's hot path.
 */
export async function detectObjects(bitmap, { model = DEFAULT_DETECTOR_MODEL, signal } = {}) {
