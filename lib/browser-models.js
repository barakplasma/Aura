...
export const BROWSER_MODELS = {
  ...
  "qwen3.5-0.8b": {
    ...
    // it is "does not finish and takes the device down" — on one phone. So it
    // is a knownIssue, not a device gate: never auto-picked or eval-matrixed,
    // but still selectable (with the warning) on hardware nobody has measured.
    knownIssue: "hung WebGPU inference on a Pixel 7a",
    sizeLabel: "~875 MB",
    downloadBytes: 875e6,
    dtype: {
      embed_tokens: "q4",
...
/**
 * A failure measured on some device (the row's `knownIssue`), or null. Such a
 * row is never auto-picked and is left out of the eval matrix, but stays
 * selectable with the warning shown.
 */
export function modelKnownIssue(key) {
  return BROWSER_MODELS[key]?.knownIssue ?? null;
}

/**
 * Why a model cannot run on *this* device, or null when it can — the picker
 * disables the option. Only device facts decide this (WebGPU fp16 the
 * adapter lacks). A failure measured on some other device is a knownIssue
 * (see modelKnownIssue), which warns instead: one phone's verdict should not
 * disable a row on hardware nobody has measured.
 */
