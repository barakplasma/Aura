    ...
    while (true) {
      ...
      try {
        ...
        result = {
          ...
          usage: scan.usage,
          // Browser-engine provenance (which runtime answered, on what device,
          // what the one-time model load cost) — undefined for provider scans,
          // preserved so the eval table can show what actually ran per cell.
          runtime: scan.runtime,
          device: scan.device,
          modelLoadMs: scan.modelLoadMs,
          error: null,
        };
      } catch (err) {
...
// `resolvePrice(model)` returns the input/output rates for each evaluated
// model, so a matrix never prices every model at the configured model's rate.
export function summarizeResults(results, expectedByImage = {}, resolvePrice = () => null) {
  const priceFor = typeof resolvePrice === "function"
    ? resolvePrice
    : () => ({ inputRate: Number(resolvePrice) || 0, outputRate: Number(resolvePrice) || 0 });
  ...
  for (const r of results) {
    ...
    agg.promptTokens += r.usage?.prompt_tokens || 0;
    agg.completionTokens += r.usage?.completion_tokens || 0;
