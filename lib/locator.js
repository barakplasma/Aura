...
function mockLocate(target) {
  ...
  return {
    ...
    y: Math.round(y),
    action: deriveAction(x, y),
    // Plausible per-frame token spend (image-heavy prompt + tiny JSON reply),
    // so the runtime cost readout is realistic without a live API key.
    usage: { prompt_tokens: 1180, completion_tokens: 18, total_tokens: 1198 },
  };
}
