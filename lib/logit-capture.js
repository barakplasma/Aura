...
/**
 * `copyRow: false` keeps only the step count and the first-step timestamp —
 * the prose legs want the timing but have no use for a ~600 KB row copy.
 * `now` is injectable for tests.
 *
 * @returns {{ state: { row: Float32Array|null, steps: number, width: number, firstStepAt: number|null },
 ...
 */
export function makeLogitCapture({ copyRow = true, now = () => performance.now() } = {}) {
  const state = {
    ...
    width: 0,
    /** When the first decode step's logits arrived: everything before it is
     *  image preprocessing, the vision encoder and prefill; everything after
     *  it is decode. That split decides whether token budgets or the image
     *  path is the latency lever. */
    firstStepAt: null,
  };
  ...
  return {
    ...
    process(_inputIds, logits) {
      state.steps++;
      if (state.firstStepAt == null) state.firstStepAt = now();
      const data = logits?.data;
      if (copyRow && !state.row && data?.length) {
