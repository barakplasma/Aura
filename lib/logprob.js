...
// evidence, and it is nearly free once generation returns scores: for the terse
// "YES" answers this family emits, that probability *is* P(verdict | frame).
 ...
 *
 * `scores`    — array of per-step logits (one row per generated token), as
 *               returned by `generate({ output_scores: true })`. Entries may be
 *               ONNX tensors (`.data`) or plain arrays.
