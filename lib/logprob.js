 ...
 * Measured on the reference phone: SmolVLM2 answers "Yes" — title case — so
 * the upper ("YES") and lower ("yes") variants resolved ids the model never
 * emitted, and the YES-vs-NO margin read 0.000 against a verdict that was
 * demonstrably in the text. Every casing the answer might arrive in is tried,
...
export function singleTokenId(tokenizer, word) {
  ...
  for (const variant of [word, ` ${word}`, lower, ` ${lower}`, title, ` ${title}`]) {
    try {
