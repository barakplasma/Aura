...
export function buildCompactDetectionPrompt(mission) {
  ...
  return [
    `Monitoring mission: "${m}"`,
    "Look at the image and decide whether that condition is happening right now.",
    "Answer on ONE line only, in this exact order: YES or NO, then a confidence",
    "number from 0 to 100, then a short reason. No JSON, no markdown, no extra lines.",
    "Example: YES 80 a person is standing at the door",
  ].join("\n");
}
...
// Accepts either the strict JSON detection schema (via isolateJsonObject) or
// a small model's loose one-line answer — "YES 80 someone at the door" —
// tolerating extra punctuation/whitespace. Never throws: unparseable input
// degrades to a conservative, non-triggered result.
export function parseLooseDetection(raw) {
  try {
    return normalizeDetection(isolateJsonObject(raw));
  } catch {
    // Fall through to loose-line parsing below.
  }
  const text = String(raw ?? "").trim();
  const firstLine = text.split("\n")[0] || "";
  const m = /^[^a-z0-9]*(yes|no)\b[\s,:;.\-]*([0-9]{1,3})?\s*%?[\s,:;.\-]*(.*)$/i.exec(
    firstLine,
  );
  if (!m) {
    return { triggered: false, confidence: 0, reason: "Nothing notable in view." };
  }
  const triggered = /^yes$/i.test(m[1]);
  const confidence = clamp(
    toNumber(m[2], triggered ? 100 : 0),
    0,
    100,
  );
  let reason = (m[3] || "").trim().slice(0, 240);
  if (!reason)
    reason = triggered ? "Alert condition met." : "Nothing notable in view.";
  return { triggered, confidence, reason };
}
