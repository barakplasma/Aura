...
export function buildCompactDetectionPrompt(mission) {
  ...
  return [
    `Monitoring mission: "${m}"`,
    "Look at the image. On ONE line, first say in a few words what you can",
    "actually see, then write YES or NO for whether the mission is satisfied",
    "right now, then a confidence number from 0 to 100.",
    "If you cannot clearly see it, answer NO. No JSON, no markdown.",
    // Observation first, verdict last, and the example answers NO: measured on
    // the reference phone with the old "Answer ... YES or NO, then ..." prompt,
    // the 500M model returned the single token "YES" for every mission put to
    // it — including "a bicycle with a front basket" pointed at a shelf of
    // books. Asking it to say what it sees before it decides is what grounds
    // the answer; the NO example counters the yes-bias the YES example fed.
    "Example: books and papers on a desk, no person NO 10",
  ].join("\n");
}
