...
export function buildDetectionPrompt(mission) {
...
export function buildActionPrompt(action, reason) {
...
export async function scan({ mission, action, image, threshold = 60, env }) {
  const cfg = getConfig(env);
  if (!cfg.apiKey) {
    return { ...mockScan({ mission, action, threshold }), mode: 'mock' };
  }

  const det = await callGemma(cfg, buildDetectionPrompt(mission), 'Assess the scene now.', image, 0.1);
  ...
  if (fired) {
    if ((action || '').trim()) {
      const act = await callGemma(
        cfg,
        buildActionPrompt(action, detection.reason),
      ...
      message = detection.reason; // no action prompt → just announce what was seen
    }
  }
...
let mockScanTick = 0;
function mockScan({ mission, action, threshold = 60 }) {
