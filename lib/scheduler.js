...
// setTimeout's delay is internally a 32-bit signed int — anything beyond this
// overflows and fires almost immediately instead of respecting the delay.
// Matters now that SCAN EVERY accepts hours: an operator entering an
// extreme value (e.g. 1,000,000 hours) must not turn into a rapid-fire loop.
const MAX_GAP_MS = 2147483647;
const clampGap = (ms) => Math.min(ms, MAX_GAP_MS);
...
function intervalGapMs(scanEvery) {
  ...
  return clampGap(
    (Number.isFinite(secs) && secs > 0 ? secs : DEFAULT_SCAN_EVERY_S) * 1000,
  );
}

// Compute the post-scan gap (ms) for the given mode.
//   knobs  — { scanEvery, budgetPerHour, networkMbPerHour, inputRate, outputRate }
//   sample — per-session EMAs { promptTokens, completionTokens, bytes, durationMs }
//
...
export function computeGapMs(mode, knobs = {}, sample = {}) {
  ...
  const { bytes, durationMs } = sample;
  const promptTokens = sample.promptTokens ?? sample.tokens;
  // Legacy callers provide one combined token EMA; a missing modern
  // completion sample must stay missing so bootstrap can still fall back.
  const completionTokens = sample.completionTokens ?? (Number.isFinite(sample.tokens) ? 0 : null);
  const dur = Number.isFinite(durationMs) ? durationMs : 0;
  const haveTokens = Number.isFinite(promptTokens) || Number.isFinite(completionTokens);
  ...
  // Cost cap: needs a token sample, both $/1M rates, and a positive budget.
  // A provider that never returns usage or a free local model simply disables
  // this cap. `rate` remains a compatibility fallback for callers upgraded
  // before the separate input/output rates were introduced.
  const inputRate = Number(knobs.inputRate ?? knobs.rate);
  const outputRate = Number(knobs.outputRate ?? knobs.rate);
  const budgetPerHour = Number(knobs.budgetPerHour);
  if (haveTokens && inputRate >= 0 && outputRate >= 0 && (inputRate > 0 || outputRate > 0) && budgetPerHour > 0) {
    const costPerScan = ((Number(promptTokens) || 0) * inputRate + (Number(completionTokens) || 0) * outputRate) / 1e6;
