...
// The old single blended rate cannot faithfully represent separate input and
// output prices. Preserve it as a per-model manual override (for both rates)
// so an existing operator's cost estimate never changes without consent.
export function migrateLegacyRate(storage) {
  if (!storage || typeof storage.getItem !== "function" || typeof storage.setItem !== "function") return false;
  if (storage.getItem("aura.pricingOverrides") != null) return false;
  let rate;
  let baseUrl;
  let model;
  try {
    rate = JSON.parse(storage.getItem("aura.rate"));
    baseUrl = JSON.parse(storage.getItem("aura.baseUrl"));
    model = JSON.parse(storage.getItem("aura.model"));
  } catch {
    return false;
  }
  const numeric = Number(rate);
  if (!Number.isFinite(numeric) || numeric < 0 || !baseUrl || !model) return false;
  const key = `${String(baseUrl).replace(/\/+$/, "").toLowerCase()}\u0000${String(model).toLowerCase()}`;
  storage.setItem("aura.pricingOverrides", JSON.stringify({ [key]: { inputRate: numeric, outputRate: numeric } }));
  return true;
}
