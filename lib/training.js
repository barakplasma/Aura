...
const detectSig = f()
  ...
  .input(
    "sceneDescription",
    f.string("Description of what is visible in the camera frame"),
  )
...
const actionSig = f()
  ...
  .output(
    "message",
    f.string("The spoken announcement or webhook payload text"),
  )
  .build();
...
function shapeDetectionExample(ex) {
  return {
    mission: ex.mission || "",
...
export async function runDetectionOptimization({
  apiKey,
  baseUrl,
  model,
  examples,
  ...
  const trainData = split.train.map(shapeDetectionExample);
  const valData = split.validation.map(shapeDetectionExample);

  const metric = ({ prediction, example }) => {
    let score = 0;
    if (prediction.triggered === example.triggered) score += 2;
    const confDiff = Math.abs(
      (prediction.confidence || 0) - (example.confidence || 0),
    );
...
export async function runActionOptimization({
  apiKey,
  baseUrl,
  model,
  examples,
