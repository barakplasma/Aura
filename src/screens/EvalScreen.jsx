...
export default function EvalScreen({
  ...
  const [activeSnapshot, setActiveSnapshot] = useState(() => snapshotActiveRun());
  ...
  const combos = useMemo(() => {
    ...
    return models.flatMap((model) =>
      runVariants.map((variant) => ({ model, variant, key: comboKey(model, variant.id) })),
    );
  }, [runRecord]);
  const resultByCell = useMemo(
    () => new Map(runResults.map((result) => [
  ...
  const summary = useMemo(
    () => summarizeResults(runResults, runRecord?.expectedByImage || expectedByImage, rate),
    [expectedByImage, rate, runRecord?.expectedByImage, runResults],
  );
  const summaryByCombo = useMemo(
    () => new Map(summary.combos.map((combo) => [comboKey(combo.model, combo.variantId), combo])),
    [summary],
  );
  const usableVariantCount = variants.filter((variant) => (variant.mission || '').trim()).length;
  const callCount = images.length * usableVariantCount * selectedModels.length;
  const disabledReason = getDisabledReason({ apiKey, images, variants, selectedModels });
  ...
  async function handleFiles(event) {
    ...
    try {
      ...
      await refreshImages();
      setStatus(`Imported ${files.length} image${files.length === 1 ? '' : 's'}.`);
    } catch (err) {
  ...
  function addManualModel() {
    ...
    if (!model) return;
    setSelectedModels(selectedModels.includes(model) ? selectedModels : [...selectedModels, model]);
    setManualModel('');
  }
  ...
  async function handleRun() {
    ...
    try {
      const results = await runEvalMatrix({
        ...
        cells,
        imagesById,
        variantsById: Object.fromEntries(usableVariants.map((variant) => [variant.id, variant])),
        concurrency,
        requestTimeout: 60,
      ...
      if (mountedRef.current) {
        setLastRun(run);
        setStatus(run.cancelled ? 'Eval cancelled. Partial results saved.' : 'Eval complete.');
      }
    } catch (err) {
    ...
    <div className="screen screen-eval">
      ...
      <section className="settings-section">
        ...
        <div className="btn-row">
          <button className="dc-btn" onClick={() => fileRef.current?.click()}>+ UPLOAD</button>
          <button className="dc-btn outline" onClick={handleCapture}>CAPTURE FRAME</button>
        </div>
          ...
          {images.map((image) => (
            <div key={image.id} className="eval-thumb">
              ...
              <div className="eval-thumb-meta">
                ...
                <button className="train-del-btn" onClick={() => deleteImage(image.id)}>x</button>
              </div>
              <div className="mode-segments eval-label-toggle">
                {[
                  ...
                  <button
                    key={label}
                    className={`mode-segment ${image.expected === value ? 'active' : ''}`}
        ...
        {variants.map((variant) => (
          <div key={variant.id} className="eval-variant">
            <div className="inline-row eval-variant-head">
              ...
              />
              <button className="dc-btn outline" onClick={() => deleteVariant(variant.id)}>DELETE</button>
            </div>
            <div className="form-group">
              ...
              <textarea
                ...
                onChange={(e) => updateVariant(variant.id, { mission: e.target.value })}
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) => updateVariant(variant.id, { instruction: e.target.value })}
                placeholder="Extra detector instruction"
              />
            </div>
          </div>
        ))}
        <button className="dc-btn" onClick={addVariant}>+ ADD VARIANT</button>
      </section>
      ...
      <section className="settings-section">
        ...
        <div className="inline-row">
          ...
          />
          <button className="dc-btn outline" onClick={addManualModel}>ADD</button>
        </div>
      ...
      <section className="settings-section">
        ...
        <div className="inline-row">
          ...
          <p className="status-msg eval-call-count">
            {images.length} images x {usableVariantCount} variants x {selectedModels.length} models = {callCount} calls
          </p>
        </div>
        <p className="status-msg">Estimated max cost from returned tokens at ${parseFloat(rate || 0).toFixed(4)} / 1M tokens.</p>
        {running && (
          <ProgressBar
            phase="processing"
            pct={progressPct}
        ...
        <div className="btn-row">
          <button className="dc-btn primary" disabled={running || Boolean(disabledReason)} onClick={handleRun}>RUN</button>
          <button className="dc-btn outline" disabled={!running} onClick={handleCancel}>CANCEL</button>
          <button className="dc-btn outline" disabled={!runRecord} onClick={exportJson}>EXPORT JSON</button>
        </div>
        {disabledReason && <p className="status-msg">{disabledReason}</p>}
        {status && <p className="status-msg" role="status" aria-live="polite">{status}</p>}
      </section>
        ...
        {runRecord ? (
          <div className="eval-table-wrap">
            <table className="eval-table">
              <thead>
                <tr>
                  <th rowSpan="2" className="eval-sticky-col">IMAGE</th>
                  {runRecord.models.map((model) => (
                    <th key={model} colSpan={runRecord.variants.length}>{model}</th>
                  ))}
                </tr>
                <tr>
                  {combos.map(({ key, variant }) => <th key={key}>{variant.name}</th>)}
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={imageId}>
                      <th className="eval-sticky-col">
                        {image?.dataUrl && <img className="eval-row-thumb" src={image.dataUrl} alt="" />}
                        <span>{labelText(expected)}</span>
                      </th>
                      {combos.map(({ key }) => {
                        const result = resultByCell.get(`${imageId}|${key}`);
                        return (
                          <td key={key} className={cellClass(result, expected)} title={result?.reason || result?.error || ''}>
                            {formatCell(result)}
                          </td>
                        );
                      })}
                ...
                <tr className="eval-agg-row">
                  <th className="eval-sticky-col">ACCURACY</th>
                  {combos.map(({ key }) => {
                    const combo = summaryByCombo.get(key);
                    return <td key={key}>{combo?.labeled ? `${Math.round(combo.labeled.accuracy * 100)}%` : '-'}</td>;
                  })}
                </tr>
                <tr className="eval-agg-row">
                  <th className="eval-sticky-col">LATENCY</th>
                  {combos.map(({ key }) => {
                    const combo = summaryByCombo.get(key);
                    return <td key={key}>{combo?.meanLatencyMs != null ? `${combo.meanLatencyMs}ms` : '-'}</td>;
                  })}
                </tr>
                <tr className="eval-agg-row">
                  <th className="eval-sticky-col">TOKENS / COST</th>
                  {combos.map(({ key }) => {
                    const combo = summaryByCombo.get(key);
                    return <td key={key}>{combo ? `${combo.totalTokens} / $${combo.estCost.toFixed(4)}` : '-'}</td>;
                  })}
                </tr>
              </tbody>
            </table>
...
function getDisabledReason({ apiKey, images, variants, selectedModels }) {
  ...
  if (variants.filter((variant) => (variant.mission || '').trim()).length === 0) {
...
function cellClass(result, expected) {
  if (!result || result.status !== 'ok' || (expected !== true && expected !== false)) {
    return '';
  }
  return result.triggered === expected ? 'eval-cell-match' : 'eval-cell-mismatch';
}
