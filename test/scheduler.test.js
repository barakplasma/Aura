...
test("interval mode accepts the full 1s..12h+ range, including fractional seconds", () => {
  ...
  assert.equal(
    computeGapMs("interval", { scanEvery: 12 * 3600 }, {}),
    43200000,
  );
});
...
test("max mode returns the 250ms floor regardless of knobs/sample", () => {
  assert.equal(
    computeGapMs("max", { scanEvery: 30 }, { tokens: 5000, durationMs: 800 }),
    250,
  );
});
...
test("budget mode bootstraps to the interval gap until a sample lands", () => {
  ...
  assert.equal(
    computeGapMs("budget", { scanEvery: 7, budgetPerHour: 0.1, rate: 0.1 }, {}),
    7000,
  );
});
...
test("budget cost cap disabled gracefully when tokens EMA is null", () => {
  ...
  assert.equal(
    computeGapMs(
      ...
      { tokens: null, bytes: 50000, durationMs: 0 },
    ),
    8000,
  );
  ...
  assert.equal(
    computeGapMs(
      ...
      { tokens: null, bytes: 100000, durationMs: 0 },
    ),
    360000,
  );
  ...
  assert.equal(
    computeGapMs(
      ...
      { tokens: 1000, bytes: null, durationMs: 0 },
    ),
    8000,
  );
});
